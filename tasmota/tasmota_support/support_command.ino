...
const char kTasmotaCommands[] PROGMEM = "|"  // No prefix
  ...
  D_CMND_ZIGBEE_BATTPERCENT "|" D_CMND_HEDERA_ACCOUNT_TOKEN "|" D_CMND_HEDERA_DEVICE_ID "|" D_CMND_HEDERA_OFFSET_NODE_API "|"
#ifdef USE_I2C
...
void (* const TasmotaCommand[])(void) PROGMEM = {
  ...
  &CmndBatteryPercent, &CmndHederaAccountToken, &CmndHederaDeviceId, &CmndHederaOffsetNode,
#ifdef USE_I2C
...
void CmndHederaAccountToken(void)
{
  ...
  if( XdrvMailbox.data_len )
  {
    SettingsUpdateText( SET_HEDERA_ACCOUNT_TOKEN, (const char*)XdrvMailbox.data);
  }

  Response_P( "{ \"D_CMND_HEDERA_ACCOUNT_TOKEN\": \"%s\" }", SettingsText(SET_HEDERA_ACCOUNT_TOKEN) );
...
void CmndHederaDeviceId(void)
{
  ...
  if( XdrvMailbox.data_len )
  {
    SettingsUpdateText( SET_HEDERA_DEVICE_ID, (const char*)XdrvMailbox.data);
  }

  Response_P( "{ \"D_CMND_HEDERA_DEVICE_ID\": \"%s\" }", SettingsText(SET_HEDERA_DEVICE_ID) );
  CmndStatusResponse(22);
  ResponseClear();
}

void CmndHederaOffsetNode(void)
{
  int32_t payload = XdrvMailbox.payload;

  if( XdrvMailbox.data_len )
  {
    SettingsUpdateText( SET_HEDERA_OFFSET_NODE_API, (const char*)XdrvMailbox.data);
  }
